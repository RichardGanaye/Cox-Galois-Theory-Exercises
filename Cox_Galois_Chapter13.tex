...
Let $k_i$ the order of multiplicity of $\beta_i$ in $\theta_f(y)$, so $\theta_f(y) = (y-\beta_i)^k q_i(y), \ q_i(y) \in L[y]$. Let $\sigma \in \Gal(L/F)$ such that $\sigma(\beta_i) = \beta_j$. Applying $\sigma$ to $\theta_f(y)$, we obtain $\theta_f(y) = (y-\beta_j)^k (\sigma \cdot q_i)(y)$, so $k_j \geq k_i$, and similarly $k_i \geq k_j$, so the distinct $\gamma_i$ have the same order of multiplicity $m$ in $\theta_f(y)$. Therefore
...
where $b,c \in F(\beta_1,\beta_2)$ are constant.
 ...
 If $b_2 \ne 0$, $\beta_2^5 = -\frac{1}{2} b_2^5$, so
...
\end{align*}
The Schšnemann-Eisenstein Criterion doesn't apply.  
...
Let $L$ the splitting field of $x^5 - 2$ over $\Q$.
 ...
 $$\Gal(K/\Q(\sqrt{5})) \simeq H \simeq \mathrm{AGL}(1,\F_5) \cap A_5.$$
...
Since $\lambda = \lambda(\alpha +1) - \lambda \alpha = \beta_1 \in K$, and $\alpha = (\lambda \alpha) / \lambda = \beta_0/(\beta_1 - \beta_0) \in K$, $F(\lambda,\alpha) \subset K$, and $\lambda \alpha,\ldots, \lambda(\alpha +4) \in F(\lambda,\alpha)$, so $K \subset F(\lambda,\alpha)$:
$$K = F(\lambda, \alpha)$$
...
then $u$ is replaced by $\zeta^2u$, and $v$ is replace by $\zeta^4 v$, therefore $\zeta^{2k}  u , \zeta^{4k} v$ is a solution of the preceding system, so $\zeta^{2k}  u + \zeta^{4k} v$ is also a root of $f$.
...
\item[(c)]  By Exercise 13, we know that the Galois group of $x^5 - 2$ over $\Q(\sqrt{5})$ is $\mathrm{AGL}(1,\F_5)\cap A_5$ which is not cyclic, so there is a misprint in the sentence. 
   ...
   \be 
\item[$\bullet$] The polynomial $f = x^5 - D \in \Q(\sqrt{5})[x], D \in \Q$, is irreducible over $\Q$ by hypothesis.  Therefore the real number $\sqrt[5]{D} \not  \in \Q$. Since $[\Q(\sqrt{5} : \Q] = 2$ and $[\Q(\sqrt{5}) : \Q] = 5$, we obtain that $10 = 2 \times 5$ divides $[\Q(\sqrt{5},\sqrt[5]{D}) : \Q]$, where $[\Q(\sqrt{5},\sqrt[5]{D}) : \Q] \leq 10$. Therefore 
$$10 = [\Q(\sqrt{5},\sqrt[5]{D}) : \Q] = [\Q(\sqrt{5},\sqrt[5]{D}) : \Q(\sqrt{5})] [\Q(\sqrt{5}):\Q],$$
 so $[\Q(\sqrt{5},\sqrt[5]{D}) : \Q(\sqrt{5})] = 5$, which proves that $\sqrt[5]{D} \not \in \Q(\sqrt{5})$. By theorem 4.2.6, $f = x^5 - D$ is irreducible over $\Q(\sqrt{5})$.
...
where $L$ is the splitting field of $x^5 - D$.
 ...
 \item[$\bullet$] Now $f= x^5 - 5Px^2-5Qx-\frac{Q^2}{P} - \frac{P^3}{Q}$. 
 
 Here we assume that $f$ remains irreducible over $\Q(\sqrt{5})$. 
...
&=(x - (u+v))(x- (\zeta^2 u + \zeta^4 v))(x- (\zeta^4 u + \zeta^3 v))(x-(\zeta u + \zeta^2 v))(x- (\zeta^3 u + \zeta v))
\end{align*}
We prove that $\alpha_2 = \zeta^2u + \zeta^4v$ is not real. If $\alpha_2 \in \R$, then $\alpha_2 = \overline{\alpha_2}$, so $(\zeta^2 u - \zeta^3)u + (\zeta^4 - \zeta) v = 0$. 
...
\end{align*}
 ...
 Therefore $\alpha_2 \not \in \Q(\sqrt{5})(\alpha_1)$, where $\alpha_1= u + v \in \R$ is a root of $f$, so $f$ doesn't split completely over $\Q(\sqrt{5})(\alpha_1)$.
