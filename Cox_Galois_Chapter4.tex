...
So $1/\alpha$ is a root of the polynomial $q = \sum\limits_{i=0}^d a_{d-i}x^{i} \in F[x]$ has $1/\alpha$, and $q\neq 0$ since $a_d \neq0$, thus $1/\alpha$ is algebraic over $F$.
\end{proof}
...
$\bullet$ Reciprocally, let $\gamma \in F[\alpha_1,\ldots,\alpha_n]$. 
...
\end{align*}
(this quotient doesn't depend on the choice of the representent $p/q$ of $f$).
...
\item[(b)]  Then there exist a BŽzout's relation between these two polynomials :
$$Ap + Bg = 1, \ A,B \in F[x].$$
...
With a reductio ad absurdum,  we suppose that $f$ is reducible over $F$ :  $f = x^2 - 3 = u v,\ u v \in \mathbb{Q}[\sqrt{2}][x]$, where $u,v$ are nonconstant polynomials. Then $\deg(u)\geq 1, \deg(v) \geq 1$, and as $\deg(u) + \deg(v) = \deg(f) = 2$, $\deg(u) = \deg(v)=1$ : 
...
\begin{enumerate}
\item[(a)]
Let $f(x) = (x-a)^n + pF(x)$, where $a\in \mathbb{Z}$, $p$ premieris prime. We show that $f$ is irreducible by reductio ad absurdum. If we suppose that $f$ is reducible over $\Q$, then by Corollary 4.2.1  $$f = g h,\ g,h \in \mathbb{Z}[x], k = \deg(g)\geq 1,l=\deg(h) \geq 1.$$
...
q= \sum_{i=0}^d a_i x^i &\mapsto \overline{q} =  \sum_{i=0}^d \overline{a_i} x^i 
 \end{align*}
is a ring homomorphism, and so  $\overline{f} = \overline{gh} = \overline{g}\,\overline{h}$.
 
Thus $$\overline{f} = (x- \overline{a})^n = \overline{g}\overline{h}$$
 
As $\deg(\overline{g}) \leq \deg(g), \deg(\overline{h}) \leq \deg(h)$ and as $\deg(\overline{g})+\deg(\overline{h}) = \deg((x- \overline{a})^n)=n = \deg(g)+ \deg(h)$, we conclude that $\deg(\overline{g}) = \deg(g) = k, \deg(\overline{h})=\deg(\overline{h})=l$.
...
As $k\geq 1, l \geq 1$, $(x-a)^k$ et $(x-a)^l$ have the root $a$, thus
...
As $i\geq 1, j\geq 1$, $u$ divides $pF - p^2 GH$ in $\mathbb{Z}[x]$ : there exists $v \in \mathbb{Z}[x]$ such that 
$$uv = p(F - pGH).$$

As $\overline{u}\, \overline{v} = 0$, and $ \overline{u} \neq 0$ in the integral domain $\F_p[x]$, then $\overline{v} = 0$ : tall the coefficients of $v$ are divisible by $p$, thus $w= v/p \in \mathbb{Z}[x]$, and
$$uw = F-pGH,\qquad  \overline{u}\, \overline{w} = \overline{F}.$$
Hencei $\overline{u} \mid \overline{F}$, in contradiction with the hypothesis $\overline{u} \wedge \overline{F} = 1$.
...
$\bullet$ Suppose this property true for $k-1$ ($1\leq k \leq p-1$) : 
$$\binom{p-1}{k-1} \equiv (-1)^{k-1} \pmod p$$
 Then, as $1\leq k \leq p-1$, we know that $\binom{p}{k} \equiv 0\  [p]$, thus from Pascal formula,
...
$F(1)\not \equiv 0 [p]$. By Exercise 2, $\Phi_p$ is irreducible.
\end{proof}

\paragraph{Ex. 4.2.4}

{\it For each of the following polynomials, use a computer to determine wheter it is irreducible over the given field.
}
...
Note : if we know the cyclotomic polynomials, as is prime :
\begin{align*}
\Phi_3(x) &= x^2+x+1\\
...
$x^2$ et $x^2+x$ are not irreducible, can be excluded of the list. It remains to test two divisons by
...
\vspace{0.5cm}
Note : the factorization of $\Phi_{15}$ over the field $\mathbb{F}_2$, gives the list of irreducible polynomials over $\mathbb{F}_2$.
\begin{verbatim}
S.<t> = GF(2)['t']
...
With a reductio ad absurdum, suppose that $f$ is reducible. By Gauss Lemma $f$ has a monic factor $g \in \mathbb{Z}[x], 1\leq \deg(g) <n$. 

 $$f =\prod_{\zeta \in \mathbb{U}_n} ( x - \zeta \sqrt[n]{a}).$$ 
 $\mathbb{C}[x]$ being a unique facoirization domain, $$g = \prod_{\zeta \in A} ( x - \zeta \sqrt[n]{a}), \emptyset \neq A \varsubsetneq \mathbb{U}_n$$
$\vert A \vert =s$ satisfies $1\leq s <n$.
...
Conclusion : $x^n -a$ is irreducibal $\mathbb{Q}$, if $a = p_1\cdots p_r$ is a product of distinct prime numbers.
...
\begin{proof}
If $f$ has a root in $k(t)$, then there exist a rational function $u/v,\ u,v \in k[t], u \wedge v = 1$ such that
...
\end{align*}
Then there exist two polynomials $a,b \in \langle f \rangle$ tsuch that $$p = \sum\limits_{k=0}^{n-1}{a_kx^k} + a = \sum\limits_{k=0}^{n-1}{b_kx^k} + b.$$
...
\begin{enumerate}
\item[(a)]
$\sqrt[4]{2}$ is a root of $p=x^4-2 \in \Q[x]$, and $p$ is irreducible over $\Q$ by Exercise 4.2.8 (or Schšnemann-Eisenstein Criterion for the prime 2). Thus $$[\Q(\sqrt[4]{2}) : \Q]=4.$$
...
\item[(b)] $\sqrt[3]{2}$ is irrational, so well that $f = x^3-2$ has no root in $\Q$, and $\deg(f)= 3$, thus $f$ is irreducible over $\Q$ and $f$ is the minimal polynomial over $\Q$ of $\sqrt[3]{2}$, and so
$$[\Q(\sqrt[3]{2}) : \Q] = 3.$$
...
$x^2-3 $ has the root $\sqrt{3}$ and its degree is 2. Its coefficients are in $\Q$, a fortiori in $\Q(\sqrt[3]{2})$. Thus the minimal polynomial $p$ of $\sqrt{3}$ over $\Q(\sqrt[3]{2})$ divides $x^2-3$ : its degree $\delta = \deg(p) = [\Q(\sqrt{3},\sqrt[3]{2}):\Q(\sqrt[3]{2}) ]$ satisfies then $\delta \leq 2$.

As $d = 3 \delta \geq 6 $, and $\delta \leq 2$, $\delta = 2$,  and so $d=6$.
...
\begin{enumerate}
\item[(a)]
$(1,\sqrt[4]{2},\sqrt[4]{2}^2,\sqrt[4]{2}^3)$ is a basis of $\Q(\sqrt[4]{2}) / \Q$, and $(1,i)$ a basis of $\Q(i,\sqrt[4]{2})/\Q(\sqrt[4]{2})$, thus
$$(1,\sqrt[4]{2},\sqrt[4]{2}^2,\sqrt[4]{2}^3, i, i\sqrt[4]{2},i\sqrt[4]{2}^2,i\sqrt[4]{2}^3)$$
is a basis of $\Q(i,\sqrt[4]{2})/\Q$

\item[(b)]
$(1,\sqrt[3]{2},\sqrt[3]{2}^2)$ is a basis of $\Q(\sqrt[3]{2})/\Q$, and $(1,\sqrt{3})$ a basis of $\Q(\sqrt{3},\sqrt[3]{2})/ \Q(\sqrt[3]{2})$, thus
$$ ( 1,\sqrt[3]{2},\sqrt[3]{2}^2, \sqrt{3},\sqrt{3}\sqrt[3]{2},\sqrt{3}\sqrt[3]{2}^2)$$
is a basis of $\Q(\sqrt{3},\sqrt[3]{2})/\Q$.
...
is a basis of $\Q\left(\sqrt{2+\sqrt{2}}\right)/\Q$.
...
is a basis of $\Q\left(i,\sqrt{2+\sqrt{2}}\right)/ \Q$.
...
\begin{enumerate}
\item[(a)]
The Scšnemann-Eisenstein Criterion with $p=2$ shows that $x^4-2$ is irreducible over $\Q$, and with $p=3$ shows that $f = x^3-3$ is irreducible over $\Q$. (Alternatively, we can use Exercise 4.2.8).

\item[(b)]

As $x^4-2$ is irreducible over $\Q$ by (a) d'aprs le (a),$x^4 - 2$ is the minimal polynomial over $\Q$ of $\sqrt[4]{2}$.
...
$\bullet$  Si $[L_i : l_{i-1}] = \infty$ pour un indice $i, 1 \leq i \leq m$, the induction hypothesis show that $[L_m : L_0] = \infty$, and as  $L_0 \subset L_m \subset L_{m+1}$, the part (a) of Theorem 4.3.8 (tower theorem), shows that $[L_{m+1} : L_0] = \infty$.

Moreover, if $[L_{m+1} : l_{m}] = \infty$, this same part (a) gives also $[L_{m+1} : L_0] = \infty$.

For all $i, 1 \leq i \leq m+1$,  $$[L_i : l_{i-1}] = \infty \Rightarrow [L_{m+1} : L_0] = \infty :$$  so the part (a) is proved for the integer $m+1$.

$\bullet$ Suppose that $[L_i : L_{i-1}] < \infty$ pour tout $i, 1\leq i \leq m+1$.
The induction hypothesis gives then

$$[L_m : L_0] = \prod_{1\leq i \leq m} [L_i : L_{i-1}]$$
The part (b) of theorem 4.3.8 imples that
\begin{align*}
...
Let $\alpha$ a root of $f$ in $\C$. Since $f$ is irreducible over $\Q$, lthe minimal polynomial of $\alpha$ over $\Q$ is $f$, and
$$[\Q(\alpha) : \Q] = \deg(f) = n.$$
...
By reductio ad absurdum, suppose that $[\overline{\Q} : \Q] < \infty$. The tower theorem gives then
...
Soit $P\in \Q[x]$ the minimal polynomial of $\alpha$ over $\Q$. Then $P$ divides $p$ in $\Q[x]$: there exists $q \in \Q[x]$ such that $p = P q$.
...
\item[(c)] 
As $\R(\alpha)$ is isomorphic to $\C$, $\R(\alpha)$ is an algebraically closed field. Moreover $L(\alpha)$ is algebraic over $\R(\alpha)$. By Exercice 4.4.7, $L(\alpha)=\R(\alpha)$.
...
\end{align*}
Reciprocally, if $\C \simeq L$, then $L(\alpha) \simeq L$. Let $\varphi : L(\alpha) \to L$ an isomorphism. Then $\beta = \varphi(\alpha)\in L$ satisfies $\beta^2 +1 = 0$, thus $\beta \not \in \R$. Consequently $\R \subsetneq L$, $1<[L : \R]\leq 2$, thus $[L:\R]= 2$.
...
$\bullet$ Reciprocally, let $\alpha \in \Q$ an algebraic integer.
...
This implies $q \mid p^n$, where $q\wedge p = 1$, thus $q \wedge p^n=1$. Hence $q\mid 1$, where $q>0$, thus $q=1$, and $\alpha = p/q = p \in \Z$.
 ...
 Conclusion : if $\alpha \in \Q$, $\alpha$ is an algebraic integer iff $\alpha \in \Z$.
