...
\begin{proof}
$L=F(\alpha_1,\cdots,\alpha_n)$, where $\alpha_i$ is algebraic over $F$. $\alpha_i$ is the root of a polynomial $p_i \in F(x)$.
...
\sigma  & \mapsto  &   (\sigma(\alpha_1), \cdots,\sigma(\alpha_n))
\end{array}
\right.
$$
...
Moreover $1_L(\sqrt{2})=\sqrt{2}, 1_L(\sqrt{3})=\sqrt{3}$, avec $1_L\in \mathrm{Gal}(L/\Q)$.
...
{\it This exercise will prove a generalizes form of Proposition 6.1.11.
\begin{enumerate}
\item[(a)] Let $\varphi:L_1\to L_2$ be an isomorphism of fields. Given a subfield $F_1 \subset L_1$, set $F_2 = \varphi(F_1)$, which is a subfield of $L_2$. Prove that the map sending $\sigma \in \Gal(L_1/F_1)$ to $\varphi \circ \sigma \circ \varphi^{-1}$ induces an isomorphism $\Gal(L_1/F_1) \simeq \Gal(L_1/F_2)$.
...
\begin{enumerate}
\item[(a)]
If $\varphi : L_1 \to L_2$ is a field isomorphism, and $\sigma \in \mathrm{Gal}(L_1/F_1)$, then ${\sigma : L_1 \to L_1}$, and so $\varphi \circ \sigma \circ \varphi^{-1}$ is a map from $L_2$ to $L_2$, composed of three field isomorphisms, is an automorphism of $L_2$.
...
Moreover, if $\chi(\sigma) = \mathrm{id}$, alors $\varphi \circ \sigma \circ \varphi^{-1} = \mathrm{id}$, 
...
If $a \neq 0$, there exists an iverse $b$ of $a$ : $ab=1$, thus $\varphi(a) \varphi(b) = \varphi(ab) = \varphi(1) = 1$, therefore $\varphi(a) \neq 0$. The kernel of $\varphi$ is null, thus $\varphi$ is injective.
...
$$\vert \mathrm{Gal}(F(\sqrt{p_1},\cdots,\sqrt{p_n})/F) \vert \leq \deg(f_1)\cdots\deg(f_n) = 2^n.$$
...
\item[(b)]
As $K \subset L$ is a finite extension, $L$ is a finite dimensional vector space over $F$. As $\sigma$ ris identity on $F$, $\sigma : L \to L$ is an injective linear application on a finite dimensional vector space, thus $\sigma$ is also surjective :   $$\sigma \in \mathrm{Gal}(L/F).$$
\end{enumerate}
\end{proof}
...
As $G$ as at least 2 elements of order d'ordre 2, since $\sigma^2 = \tau^2 = 1_L$. This is not the case in  $\Z/4\Z$. Thus 
...
\item[(b)]
$L$ is the splitting field of the separable irreducible polynomial $g = x^3-2\in \Q[x]$. Indeed, $g$ is irreducible over $\Q$ since $\deg(g) = 3$ and $g$ has no root in $\Q$,and separable since its roots in $\C$ are $\sqrt[3]{2}, \omega\sqrt[3]{2},\omega^2\sqrt[3]{2}$ which are distinct.
...
\item[(b)]
$L$ is the splitting field of the separable irreducible polynomial $g = x^5-2\in \Q[x]$ over $\Q$. Indeed,  $g$ is  irreducible over $\Q$ by Schšnemann-Eisenstein Criterion with $p=2$, and separable since its roots in $\C$ are $\sqrt[5]{2}, \zeta\sqrt[5]{2},\zeta^2 \sqrt[5]{2},\zeta^3 \sqrt[5]{2},\zeta^4\sqrt[5]{2}$ which are distinct.
...
If we apply this result to $\zeta_n^i =\sigma(\zeta_n)$, on obtient
...
\item[(e)]
As $\Q(\zeta_n)$ is the splitting field of a separabel polynomial over  $\Q$, $$\vert  \Gal(\Q(\zeta_n)/\Q)\vert = [\Q(\zeta_n):\Q].$$

If we suppose that $ [\Q(\zeta_n):\Q]= \phi(n)$,  $\varphi$is an injection between two set of same cardinality, thus $\varphi$ is a bijection, and so$\varphi$ is a group isomorphism. Reciprocally, if $\varphi$ is a group isomorphism, then $ [\Q(\zeta_n):\Q] = \Gal(\Q(\zeta_n)/\Q)\vert = \phi(n)$
...
(ViertelGruppe de Klein : cf Exercise 6.2.1 and example 6.2.2 for more details).
...
$f$ is irreducible over $\Q$ by the Schšnemann-Eisenstein Criterion with $p=2$.
...
(En effet, $\tilde{\sigma}^{-1} \tilde{\tau} = (1,4,3,2) (2,4)  = (4,3,2,1) =(2,4) (1,2,3,4)= \tilde{\tau} \tilde{\sigma}$.)
...
If $\sigma \in \Gal(L : \Q)$, $\sigma(i) = \pm i, \sigma(\sqrt{2}) = \pm \sqrt{2}, \sigma(\sqrt{3}) = \pm \sqrt{3}$. As $\vert \Gal(L : \Q) \vert = 8$, all of these  possibilities occur: there exist 8 $\Q$-automorphisms of $L$ satisfying thes equalities. As $L = \Q(i,\sqrt{2},\sqrt{3})$, $\sigma \in \Gal(L : \Q)$ is uniquely determined by the images of these 3 elements.
...
In particular, there exists so $\sigma (= \sigma_1) \in \Gal(L/\Q)$ defined by 
$\sigma(\alpha) = \beta$.
...
$\Gal(L/\Q)$ is so cyclic, generated by  $\sigma$ :
...
Let $M=F(\alpha_1,\cdots,\alpha_r), N = F(\beta_1,\cdots,\beta_{s})$, then $M$ is a splitting field of $g$ over $F$, $N$ a splitting field of $h$ over $F$, and  $L=F(\alpha_1,\cdots,\alpha_{r}, \beta_1,\cdots,\beta_{s})$ is a splitting field of  $f$ over $F$. As $f$ is separable, the $d=r+s$ roots of $f$, $\alpha_1, \cdots,\alpha_{r},\beta_1,\cdots,\beta_{s}$ are distinct.
...
Let $\sigma \in \Gal(L/F)$. As $g,h \in F[x]$, $\sigma$ induce a permutation of the roots of $g$ and of the roots of $h$, so the maps
\begin{center}
$\sigma_1 : 
\left\{
\begin{array}{ccc}
...
$
\end{center}
restrictions  of $\sigma$ ˆ $A,B$, satisfy $\sigma_1 \in S(A), \sigma_2 \in S(B)$.
...
Comme $L=F(\alpha_1,\cdots,\alpha_{r}, \beta_1,\cdots,\beta_{s})$, $\sigma = 1_L$.
...
\begin{proof}
A subgroup $H$ of $S_n$ defines an action on $[1,n]$ by $h\cdot x = h(x), h\in H, x \in [1,n]$. By definition $H$ is a transitive subgroup of $S_n$ if this action is transitive, i.e. if the only orbit is ${\cal O}_i = [1,n],\ i=1,\cdots,n$.
...
So every $v \in \F_p$ has a unique antecedent, therefore $\gamma_{a,b}$ est bijective.
...
$$g(x_1,\cdots,x_n)  = \gamma. f = f(x_{\gamma(1)},\cdots,x_{\gamma(n)}).$$
...
$$\tau\cdot(\gamma \cdot f) = f(x_{(\tau\gamma)(1)},\cdots,x_{(\tau\gamma)(n)}) = (\tau\gamma) \cdot f.$$
...
If $f = p/q \in K$, then the fraction $\phi(p)/\phi(q)$ doesn't depends of the choice of the representent $(p,q)$ rof the fraction: if $f = p/q = r/s$, then $ps=qr$, thus $\phi(p)\phi(s) = \phi(ps) = \phi(qr) =\phi(q)\phi(r)$, and so $\phi(p)/\phi(q) = \phi(r)/\phi(s)$. Therefore there exists a map $\tilde{\phi} : K \to K$ defined for all $p/q\in  K$ by
...
\begin{proof}
Let $G_n$ lthe subgroup of $S_n$ generated by the transpositions $(1,2),(2,3),\cdots,{(n-1,n)}$:
$$G_n=\langle (1,2), (2,3),\cdots,(n-1,n) \rangle$$
 For all $i\in \{1,\cdots,n\}$, there exists $g\in G_n$ such that $g(n) = i$.

Indeedt, if  $g = (i,i+1) \circ (i+1,i+2)\circ \cdots \circ (n-1,n)  =  (i,i+1) (i+1,i+2)\cdots (n-1,n)$  (with the convention $g=e$ if $i=n$).

Then  $g \in G_n$ and $g(n) = i$
(as ${\cal O}_n = [1,n]$, $G_n$ is a transitive subgroup of $S_n$).

Conclusion :for all $i \in \{1,\cdots,n\}$, there exists $g \in G_n$ tel que $g(n) = i$.
...
Conclusion : for all $n\geq 2$, $S_n = \langle (1,2), (2,3),\cdots,(n-1,n) \rangle$

\bigskip

We recall the following lemma :

{\bf Lemma : }{\it If $g=(a_1,\cdots,a_k)$ is a cycle in $S_n$, and $\sigma \in S_n$, then

$$\sigma \circ g \circ \sigma^{-1} = (\sigma(a_1),\cdots, \sigma(a_k)).$$}
...
$\bullet$ if $x \not \in \{\sigma(a_1),\cdots,\sigma(a_k)\}$, then $\sigma^{-1}(x) \not \in \{a_1,\cdots,a_k\}$,

therefore $g(\sigma^{-1}(x)) = \sigma^{-1}(x), (\sigma \circ g \circ \sigma^{-1})(x) = x$. $\qed$

\bigskip

Let $\tau = (1,2), \sigma = (1,2,\cdots,n)$.
...
{\it Proof of lemma.}  $\beta \in S_p$ is a $p$-cycle, so $\beta = (a_1,a_2,\cdots,a_p) = (a , \beta(a), \cdots, \beta^{p-1}(a))$, 
...
Consequently $\beta^k = (a, \beta^k(a), \beta^{2k}(a), \cdots,\beta^{(p-1)k}(a))$ is a $p$-cycle, thus $i$ is in the support of $\beta^k$.

$\alpha= (i,j), \beta^k = (i , j = \beta^k(i), \cdots,\beta^{(p-1)k}(i) )$ generate  $S_n$ as in Exercice 7 where we have proved that $\sigma = (1,2,\cdots,p)$ and $\tau = (1,2)$ generate $Sp$.
...
\begin{array}{cccc}
  1 & 2  & \cdots & p \\
   i &  j = \beta^k(i) & \cdots & \beta^{k(p-1)}(i) 
\end{array}
\right).
$$
...
The  Schšnemann-Eisenstein Criterion with $p=5$ shows that $f$ is irreducible over $\Q$ (if  $f= \sum_{k=0}^5 a_i x^i$, $5\nmid a_5=2, 5\mid a_i, i=0,\cdots 4, 5^2\nmid a_0 = 5$).
...
\item[$\bullet$] First proof (without Galois groups).
...
If $g$ is the minimal polynomial of $\sqrt[p]{2}$ over $\Q(\zeta_p)$, then ${\deg(g) = [\Q(\zeta_p,\sqrt[p]{2}) : \Q(\zeta_p)] } = p$. Moreover $\sqrt[p]{2}$ is a root of $f=x^p-2\in \Q[x] \subset\Q(\zeta_p)[x]$, thus $g \mid f$ in $\Q(\zeta_p)[x]$, where $f,g$ are of the same degree  $p$ and monic, thus $g=f = x^p-2$.

Conclusion : $x^p-2$ is irreducible over $\Q(\zeta_p)$.



\item[$\bullet$] Second proof (with Galois groups).

In $L$, $f$ splits entirely under the form $x^p-2 = \prod\limits_{k=1}^p (x- \alpha_k)$, where $\alpha_k = \zeta_p^{k-1} \sqrt[p]{2}$.

Let $g$ the minimal polynomial of $\sqrt[p]{2}$ over $\Q(\zeta_p)$. As $g$ divides $x^p-2$, the set of its roots is a subset of $\{\alpha_1,\cdots,\alpha_p\}$.

By Section 6.4, there exists $\sigma \in \Gal(L/\Q)$ such that $\sigma(\zeta_p) = \zeta_p, \sigma(\sqrt[p]{2})=\zeta_p\sqrt[p]{2}$.

$\sigma= \sigma_{1,1}$ maps to $ \mathrm{AGL}(1,\F_p)$ on the translation $u \mapsto u+1$, which generates the subgroup $T$ with  $p$ elements.

Since $\sigma(\zeta_p) = \zeta_p, \sigma \in \Gal(L:\Q(\zeta))$ fixes the polynomial $g$, thus sends the roots of $g$ on the roots of $g$. This remains true for the powers $\sigma_{1,k} = \sigma^k$ of $\sigma$.Therefore, $\alpha_1 = \sqrt[p]{2}$ being a root of $g$, its successive images by $\sigma$ are  $\alpha_2,\cdots,\alpha_{p-1}$, which are so roots of $g$,  and so $f\mid g, f = g$. Therefore $x^p-2$ is irreducible over $\Q(\zeta_p)$.

Note : $\Gal(L/\Q(\zeta)) \simeq T \subset\mathrm{AGL}(1,\F_p)$, thus $\Gal(L/\Q(\zeta)) \simeq \Z/p\Z$.
\ee
\end{proof}
...
$\bullet$ Reciprocally, suppose that for all $i$, $1\leq i \leq n$, $\alpha_i = \theta_i(\alpha), \theta_i \in F(x)$.
Thus $\alpha_i\in F(\alpha)$. Consequently $L=F(\alpha_1,\cdots,\alpha_n)\subset F(\alpha)$. 

As $F(\alpha) = F(\alpha_1)\subset F(\alpha_1,\cdots,\alpha_n)$, $L =F(\alpha_1,\cdots,\alpha_n) = F(\alpha)$.
...
Let $L=F(\alpha_1,\cdots,\alpha_n)$ a splitting field of $f \in F[x]$, et $\alpha=\alpha_1$.

By definition of an Abelian equation,  there exists $\theta_i \in F(x)$ tels que $\alpha_i = \theta_i(\alpha)\ (i=1,\cdots,n)$, so $L = F(\alpha)$ (see Exercise 1).
...
RŽciproquement, si $\sigma( \tau (\alpha))  = \tau(\sigma(\alpha))$, then $(\sigma \tau)(\alpha) = (\tau \sigma)(\alpha)$.
...
Conversely, suppose that the roots $\alpha_1,\cdots,\alpha_n$ of any Abelian equation $f=0$ in the splitting field of $f$ can be expressed rationally in terms of a root of unity
 $\zeta_n$, and suppose also (a) : $\Q\subset L \subset \C$, the extension $\Q\subset L$ is normal, and $\Gal(L/\Q)$ is an Abellan group.
 
 As the characteristic of $\Q$ is 0, $\Q\subset L$  is also separable, and there exists a primitive element $\alpha$ for the extension  $\Q \subset L$. Let $f$ the minimal polynomial of $\alpha$ over $\Q$. By Exercise 6, since $\Q\subset L$ is normal and separable, the equation $f=0$ is Abelian. By hypothesis, the roots  $\alpha_1=\alpha,\cdots,\alpha_n$ of $f$ can be expressed rationally in terms of a root of unity $\zeta_n$, therefore $\alpha_i \in \Q(\zeta_n), 1\leq i \leq n$. In particular $\alpha \in \Q(\zeta_n)$, thus $L = \Q(\alpha) \subset \Q(\zeta_n)$. (b) is so proved under the hypothesis (a).
