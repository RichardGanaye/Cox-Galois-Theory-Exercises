...
is of order $p$, and corresponds to the $p$-cycle $\tilde{\sigma} = (1, 2, \cdots, p)$, if we number the roots by $z_1 = \sqrt[p]{2}, \ldots,  z_p= \zeta_p^{p-1} \sqrt[p]{2}$. Since $\tilde{\sigma}^{k-1}(1) = k, k = 1, \ldots, p-1$, the subgroup $\langle \tilde{\sigma} \rangle \subset S_n$ is transitive, and so is $\langle \sigma \rangle$. Since $\Gal(L/\Q(\zeta_p)) \supset \langle \sigma \rangle$, $\Gal(L/\Q(\zeta_p))$ act transitively on the roots of $x^p - 2$. So, by Proposition 6.3.7, $x^p - 2$ is irreducible over $\Q(\zeta_p)$.
