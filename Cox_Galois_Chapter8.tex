...
If $\sigma \in S_4$, $\sigma  a \sigma^{-1} = \sigma [(1\,2)(3\,4))] \sigma^{-1}= (\sigma(1)\, \sigma(2)) (\sigma(3)\, \sigma(4)) \in K$, and the same is tur for $b$ and $c$, so $K$ is normal in $S_4$ (a fortiori in $A_4$).

Conclusion : $K =  \{e,(1\, 2)(3\, 4), (1\, 3)(2\, 4), (1\, 4)(2\, 3)\}$ is a normal subgroup of $S_4$ included in $A_4$, isomorphic to $\mathbb{Z}/2 \mathbb{Z} \times \mathbb{Z}/2 \mathbb{Z}$.
...
where ${G}_i \lhd {G}_{i-1}$, and $ {G}_i / {G}_{i-1}$ is of prime order.
\be
...
$\bullet$ We know that $G_i \lhd G_{i-1}$. We show that $\tilde{G}_i \lhd \tilde{G}_{i-1}$.
...
As $G_i \subset \ker(\varphi)$,  if two elements $g,g'$ of $G_{i-1}$ are congruent modulo $G_i$,  $gG_i = g' G_i$ then $g^{-1} g' \in G_i \subset \ker(\varphi)$, so $g,g'$ have the same image by $\varphi$. Consequently $\varphi(g)$ depends only of the class $gG_i$ of $g$ in $G_{i-1}/G_{i}$.

The mapping $\overline{\varphi} : G_{i-1}/G_i \to \tilde{G}_{i-1}/\tilde{G}_i$ defined by $g G_i \mapsto \pi(g)\tilde{G}_i$ is so well defined, and is a group homomorphism.

Let $y = v \tilde{G}_i$ any element of $\tilde{G}_{i-1}/\tilde{G}_i$, where $v \in \tilde{G}_{i-1}$, so $v = \pi(g)$ for some $g \in G_{i-1}$. Therefore $y = \pi(g) \tilde{G}_i = \varphi(g) = \overline{\varphi}(gG_{i})$, so $\overline{\varphi}$ is surjective.


\item[(b)] Let $\phi : M_1 \to M_2$ a surjective group homomorphism, and suppose that $\vert M_1 \vert = p$ is prime.
...
\item[(c)] 
By hypothsis $|G_{i-1}/G_{i}\ = p$ is prime. By part (b) applied to the surjective group homomorphism $\phi = \varphi:G_{i-1}/G_i \to \tilde{G}_{i-1}/\tilde{G}_i$, we know that $\vert \tilde{G}_{i-1}/ \tilde{G}_{i} \vert = 1$ or $p$.
...
so $\pi(xyx^{-1}) = \pi(x) \pi(y) \pi(x^{-1})  \in \tilde{G}_{i}$, so $xyx^{-1} \in \pi^{-1}(\tilde{G}_i) = G_i$.
...
The stabilizer of $x$ is the set of  $g \in G$ such that $g^{-1} x g = x$, soit $xg = gx$: this is the normalizer $C_x$ of $x$.
...
\begin{proof}
\be
\item[(a)] Let $G$ a group of order 10, and $N$ the number of 5-Sylow subgroups of $G$. Then $N\equiv 1\pmod 5$ and $N\mid 10$. Therefore $N= 1$. A group of order 10 has a unique 5-Sylow $H$, and as all 5-Sylow subgroup are conjugate, $H$ is normal in $G$, since the conjugate of a 5-Silow is a 5-Sylow, . Then $\vert H \vert= 5$ and $\vert G/H\vert = 2$ are prime, so $H$ and $G/H$ are cyclic, which implies that $G$ is solvable.

Same reasoning if $\vert G \vert = 15$: then $N \equiv 1 \pmod 5$, and $N \mid 15$, therefore $N=1$.

\item[(b)] Let $G$ a group of order 30.
\be
...
\item[(d)] Using a reductio ad absurdum, suppose that  $G$ contains ten 3-Sylow subgroups of $G$ and six  5-Sylow subgroups.

Two distinct cyclic subgroups of order $p$, with $p$ prime, have a trivial intersection, otherwise any non trivial element in the intersection would be a generator of these two subgroups, which would be then identical.
...
$u^3 = z^3 + \frac{1}{z^3} + 3(z + \frac{1}{z})$, donc $z^3 + \frac{1}{z^3} = u^3 - 3u$.
...
These 3 roots of $f$ are distinct, since $f$ sont the function $\cos$ is strictly decreasing on $[0,\pi]$.
...
$$\mathbb{Q} \subset L \subset \mathbb{Q}(\zeta).$$

As $\zeta_7^7 = 1 \in \mathbb{Q}, \mathbb{Q}(\zeta)$ is by defintion a radical extension of $\mathbb{Q}$, so $\Q \subset L$ is a solvable extension.
\ee
\end{proof}
...
Using a reductio ad absurdum, suppose that the extension $\mathbb{Q}\subset L$ is radical.
...
Let $p$ the minimal polynomial of $\gamma$ over $\mathbb{Q}$. Then $\deg(p) = [L:\mathbb{Q}] = 3$.  As $\gamma$ is a root of $x^m - \gamma^m \in \mathbb{Q}[x]$, $p$ divides $x^m - \gamma^m$.
...
Conclusion : $\mathbb{Q}\subset L$ is not a radical extension (but $\mathbb{Q} \subset L \subset \mathbb{Q}(\zeta_7)$, so $L/\mathbb{Q}$ is solvable).
\end{proof}
...
\begin{proof}
\be
\item[(a)] Let $A$ the set of the subfields of $L$ containing $K_1$ and $K_2$. Then $A \neq \emptyset$, since $L \in A$.
...
Conversally, as $K = F(\alpha_1,\alpha_2,\cdots,\alpha_n,\beta_1,\beta_2,\cdots,\beta_m)$ is a subfield of $L$ containing $K_1 = F(\alpha_1,\alpha_2,\cdots,\alpha_n)$ and $K_2 = F(\beta_1,\beta_2,\cdots,\beta_m)$, $K \in A$, so $K$ contains the smallest element of $A$, which is $K_1K_2$.
...
$\bullet$  $F\subset K$ is a Galois extension (as previously proven) and $K \supset L$.
...
Moreover, if $L'' = F(\alpha_i)$, for any $i=1,\ldots,r$, we know that there exists $\sigma \in \mathrm{Gal}(M/F)$ such that $\sigma(\alpha) = \alpha_i$ (Prop.5.1.8). As previously proven, $L'' =  F(\alpha_i) = \sigma(F(\alpha)) = \sigma(L)$.
...
Using an induction, if we suppose that $F_{i-1} \subset F'_{i-1}, 1\leq i < n$, then $F_{i-1}(\gamma_i) \subset F'_{i-1}(\gamma_i)$,  therefore $F_i \subset F'_i$.
...
$$ K_1K_2 = F'_n.$$
So we know that $K_1K_2$ is a radical extension of $K_2$.
...
Consequently $\sigma L/F$ is a radical extension.
\end{proof}
...
So the Galois closure exists: let $M$ a Galois closure of $F \subset K_1K_2/F$ , and $\sigma$ any element of $\Gal(M/F)$.
...
By Theorem 7.2.5, this is sufficient to $K_1K_2$ be a Galois extension of $F$.
\end{proof}
...
\begin{enumerate}
\item[(a)]
Let $f=x^m-1,\ m\in \N^*$. Then $f'=m x^{m-1}$ is relatively prime with  $f$. Indeed $m\neq 0$ in $L$ since the characteristic of $L$ is not $0$, and $-f +m^{-1} x f'= -x^m+1 +1 x^m = 1$ is a BŽzout's relation between $f$ et $f'$. Therefore (Prop. 5.3.2), $f$ is a separable polynomial, and $x^m-1$ has so $m$ distinct roots in $M$,  the splitting field of $f$ over $L$.

\item[(b)] We show that $\mathbb{U} = \{\alpha \in M\ \vert \ \alpha^m=1\}$, the set of the $m$ roots of $f$ in $M$, is a subgroup of $M^*$.
\be
\item[$\bullet$] $1^m = 1$, donc $1\in \mathbb{U} \neq \emptyset$.

\item[$\bullet$] Si $\alpha, \beta \in \mathbb{U}$, alors $(\alpha \beta^{-1})^m = \alpha^m (\beta^m)^{-1} = 1$, donc $\alpha \beta^{-1} \in \mathbb{U}$.
\ee
...
Thus $(\sigma \tau)(\gamma_i) = \sigma(\zeta_i^k \gamma_i) = \sigma(\zeta_i)^k \sigma(\gamma_i) = \zeta_i^k \sigma(\gamma_i)$, since $\zeta_i \in F$, therefore $\zeta_i \in F_i$.
 
  $(\sigma \tau)(\gamma_i) =  \zeta_i^k   \zeta_i^l \gamma_i = \zeta_i^{l+k} \gamma_i$, donc $\varphi(\sigma \tau) = [l+k] = [l]+[k] = \varphi(\sigma) + \varphi(\tau)$.
  
  $\bullet$ $\varphi$ is an injective homomorphism: 
  
if $\sigma \in \ker(\varphi)$, $[l] = [0]$, so $\sigma(\gamma_i) = \sigma_i^l \gamma_i = \gamma_i$. As $\sigma$ fixes the elements of $F_{i-1}$ and also $\gamma_{i}$, this is the identity on $F_i=F_{i-1}(\gamma_i)$. $\ker(\varphi) = \{e\}$, and $\varphi$ is injective.
  
\item[(c)]
Therefore $\Gal(F_i/F_{i-1})$ is isomorphic to a subgroup $H$ de $\Z/m_i\Z$. As every subgroup of a cyclic group is cyclic, $H$ is cyclic, so 
\begin{center}
...
{\it Proof of Lemma 2:}   Using the induction hypothesis $\vert c_1\cdots c_k \vert = p_1\cdots p_k, \ k<r$, and applying Lemma 1 to $a=c_1\cdots c_k$ d'ordre  $p_1\cdots p_k$, and $b = c_{k+1}$ of order $p_{k+1}$, where $p_1\cdots p_k \wedge p_{k+1}=1$, then $\vert c_1\cdots c_{k+1} \vert  = p_1\cdots p_{k+1}$. \qed
...
So the condition 8.11 is satisfied: $F$ contains a  $p$th primitive root of unity for all $p$ dividing $\Gal(L/F) = [L:F]$. The part $(b)\Rightarrow(a)$ in the proof of Theorem 8.3.3 shows then that $F\subset L$ is a radical extension.
...
\be
\item[(a)] Use the comutational methods of Section 2.3 the formulas for $\alpha_1^3$ and $\beta_1$ stated in the text.
\item[(b)] Prove (8.15).
\ee
}
...
&+6x_1x_2x_3,
\end{align*}
 thus  $\alpha_1$ is of the form
$$\alpha_1 = p + 3\omega r + 3 \omega^2 s + 6q$$
where
\begin{align*} 
...
\end{align*}
$r,s$ are fixed by the permutations of $A_3$ must be of the form $A+B\sqrt{\Delta}, \ A,B\in K = \Q(\sigma_1,\sigma_2,\sigma_3)$, where we choose
\begin{align*}
...
In deed the same calculation gives $\beta_1$, by exchanging $x_2$ with $x_3$, which transforms $\sqrt{\Delta}$ in  $- \sqrt{\Delta}$.
...
$\bullet$ If $\{i,j\} = \{k, l\}$, alors $(i,j)(k,l) = (i j)^2 = ()= e$ is the empty product.
...
{\bf Theorem.} {\it The alternating group $A_n$ is solvable for all $n\geq 5$.}
...
This proves that this commutator is a permutation in $H$ that move at most 6 elements of $\{1,\ldots,n\}$, the elements $j_1,j_2,j_3,\sigma^{-1}(j_1),\sigma^{-1}(j_2),\sigma^{-1}(j_3)$.

\item[(b)] 
\begin{enumerate}
\item[$\bullet$] {\bf Cas 1. }  First suppose that one of the cycles in the cycle decomposition of $\sigma$ has a length $\geq 4$, say
...
\end{align*}
We obtain well the permutation $(1\, 5\, 3)$.
...
Let $G$ be a finite group, and $H\neq G$ a normal subgroup of $G$ of maximal order. We show that $G/H$ is a simple group.
 ...
 Let $\pi = G \to G/H, g \mapsto gH$ lthe canonical projection, and $K =\pi^{-1}(\overline{K})$.
...
We suppose the induction hypothesis that every group with order less that $n$ has a composition series, and let $G$ be a group of order $n$.

If $G$ est simple, $\{e\} \subset G$ is a composition series for $G$. Otherwise $G$ has a subgroup $H$ such that 
$$\{e\} \subsetneq  H  \subsetneq G, \qquad H \lhd G.$$

As $|G/H| < n$, the induction hypothesis applied to $G/H$ gives a composition series

$$\{\overline{e}\}=\overline{G}_{m} \subset \overline{G}_{m-1} \subset \cdots \subset \overline{G}_{1} \subset \overline{G}_{0} = G/H,$$
with $\overline{G}_{i} \lhd \overline{G}_{i-1}, \ i=1,\cdots,m$ and $\overline{G}_{i-1}/\overline{G}_{i}$ simple.

Call $G_i = \pi^{-1}(G_i), \ i=1,\cdots,m$. As $\pi$ is surjective, $\overline{G}_i = \pi(G_i)$. Then
$$H = G_m \subset G_{m-1} \subset\cdots \subset G_1 \subset G_0 = G.$$

Moreover, $\overline{G}_{i} \lhd \overline{G}_{i-1}$ implies that $G_i \lhd G_{i-1}$ with the same proof as in part (a): if  $y \in G_{i-1}, x \in G_i$, then $\overline{y} = yH \in\overline{G}_{i-1}$ and $\overline{x} = xH \in \overline{G}_i$, with $ G_{i} \lhd G_{i-1}$, hence $\pi(yxy^{-1}) = \overline{y}\,\overline{x}\,\overline{y}^{-1} \in \overline{G_i}$, 	nd $yxy^{-1} \in G_i = \pi^{-1}(\overline{G}_i)$.

Moreover $$G_{i-1}/G_{i} \simeq \overline{G}_{i-1} / \overline{G}_{i}.$$
Indeed, the mapping $\varphi : G_{i-1} \to \overline{G}_{i-1} / \overline{G}_{i}$ defined by $g \mapsto \varphi(g) = \pi(g) \overline{G}_i$ is a group homomorphism, and $\varphi$ is surjective: if $h \overline{G}_i,\ h\in \overline{G}_{i-1}$ is any element of $\overline{G}_{i-1} / \overline{G}_{i}$, then $h=\pi(g)$ for some $g \in G_{i-1}$, since $\overline{G}_{i-1} = \pi(G_{i-1})$, hence $h\overline{G}_i = \varphi(g)$.

For all $g\in G_{i-1}$,
$$g \in \ker(\varphi) \iff \pi(g) \overline{G}_i =  \overline{G}_i \iff \pi(g) \in \overline{G}_i \iff g \in \pi^{-1}(\overline{G}_i) = G_i,$$ hence $\ker(\varphi) = G_i$.

By the first Isomorphism Theorem, $G_{i-1}/G_{i} \simeq \overline{G}_{i-1} / \overline{G}_{i}$, so $G_{i-1}/G_{i}$ is a simple group.

As $|H|<n$ , the induction hypotheisis gives a composition series for $H$, that we write
$$\{e\} = G_l \subset G_{l-1} \subset \cdots \subset G_m=H.$$

If we glue together these two composition series, we obtain 
$$\{e\} = G_l \subset G_{l-1} \subset \cdots \subset G_m=H  \subset G_{m-1} \subset\cdots \subset G_1 \subset G_0 = G$$
which is a composition series for $G$, and the induction is done.
...
As $\vert G \vert$ is $(G_{i-1} : G_i)$ is odd for all $i,\ i=1,\cdots,m$.
...
Let $\zeta$ be a $m$th primitive root of unity, where $m =[L_1:F]$. Write $M_1 = L_1(\zeta)$.  As $F \subset L_1$ is a solvable Galois extension, by Corollary 8.3.4,  $F \subset M_1$ is a radical extension, so there exist fiels $F_i, i=1,\cdots,m$ such that 
$$F_0 = F \subset F_1\subset \cdots \subset F_{n-1} \subset F_m =M_1=L_1(\zeta), $$
and $F_i = F_{i-1}(\gamma_i), \gamma_i \in F_i, \gamma_i^{m_i} \in F_{i-1},m_i>0\  (i=1,\cdots,m)$.

Moreover, $M_1 = L_1(\zeta)$ is the splitting field of $x^m-1$ over $L_1$. Let $M_2 = L_2(\zeta')$ the splitting field of $x^m-1$ over $L_2$. By theorem 5.1.6, there exists an isomorphism $\overline{\varphi} : M_1\to M_2$ such that $\varphi = \overline{\varphi}\vert_{L_1}$.

Write $F'_i = \overline{\varphi}(F_i)$. Then $M_2$  is such that $F \subset L_2 \subset M_2$, and $M_2$ is an extension of $L_1$ such that $L_2$
$$F'_0 = F \subset F'_1\subset \cdots \subset F'_{n-1} \subset F'_m =M_2,$$
and $F'_i = F'_{i-1}(\gamma'_i)$, where $\gamma'_i = \varphi(\gamma_i) \in F'_{i}$ satisfies ${\gamma'_i}^{m_i} = \varphi(\gamma_i)^{m_i} = \varphi(\gamma_i^{m_i}) \in \varphi(F_{i-1}) = F'_{i-1}$.
...
$\bullet$ Let $M$ any extension of $F(\alpha)$such that $M$ is Galois over $F$. As $\alpha$ is a root of the irreducible polynomial $f\in F[x]$, and $\alpha \in M$, where $M$ is a normal extension of $F$, the polynomial $f$ splits completely over $f$ and its distinct roots $\beta_0 = \alpha = \alpha_0, \beta_1,\cdots,\beta_n$ are in $M$.
Let $L'= F(\beta_0,\cdots,\beta_n)$. $L'$ is a splitting field of $f$  over $F$, hence there exists an isomorphism  $\varphi : L\to L'$ which is the identity on $F$, that is an embedding of $L$ in $M$ which is the identity on $F$.
...
\begin{proof}
By Proposition 5.3.8, $f$ has the same roots as $g = f/\mathrm{pgcd}(f,f')$. The splitting field $L$ of $f$ over $F$ is so the splitting field of the separable polynomial $g$. As $\deg(g) \leq \deg(d) \leq 4$, $L$ is solvable over $F$ by Proposition 8.5.4. We can conclude that all polynomial $f \in F[x]$ of degree $n\leq 4$, separable or not, is solvable by radicals over $F$.
\end{proof}
...
So $F \subset M$ and $M \subset MN$ are radical extensions, so $F \subset MN$ is a radical extension (Lemma 8.3.7(c)).

As $\alpha \in N \subset MN$, with $F \subset MN$ radical, by definition $\alpha$ is expressible par radicals over $F$ and by Proposition 8.5.2, it minimal polynomial $f$ over $F$ is solvable by radicals over $F$.
\end{proof}
...
$[L_0:M_0] = [L:M]=p$. Reasoning by induction, we suppose that $M_{i-1}\subset L_{i-1}$ is a Galois extension and $[L_{i-1} : M_{i-1}] = p$ (where $p$ is the odd prime  $[L:M]$). As $L_i = L_{i-1}(\gamma_i), M_i =M_{i-1}(\gamma_i),\ i=1,\cdots,n$, the Exercise 8.6.4(a) shows that $M_{i}\subset L_{i}$ is Galois, and the proof of Proposition 8.6.4 shows that $[L_i : M_i] = [L_{i-1}(\gamma_i) : M_{i-1}(\gamma_i)] = p, \ i=1,\ldots,n$, therefore 
...
By the Theorem of the Primitive Element (Theorem 5.4.1), as $M \subset L$ is Galois, the exists a separable element $\delta \in L$ such that $L = M(\delta)$. Let $f$ be the minimal polynomial of $\delta$ over $M$. Then $f \in M[x]$ is a separable polynomial, and as $M \subset L$ is normal, $f$ splits completely over $L$. Write $\delta_1 = \delta, \delta_2,\ldots,\delta_n$ the roots of $f$ in $L$. 

Then $$L(\gamma) = M(\delta,\gamma) = M(\gamma) (\delta).$$
Moreover, as $\delta_i \in L \subset M$, with $\delta = \delta_1$, then $M(\gamma) (\delta) = M(\gamma)(\delta_1,\ldots \delta_n)$, so
...
\begin{proof}
\item[(a)] $$\beta^n-\beta + t = 0, \qquad \beta = \frac{A}{B},$$
so
$$\frac{A^n}{B^n} - \frac{A}{B} + t = 0,$$
$$A^n - A B^{n-1} + tB^n = 0.$$
Hence $B \mid A^n$, and $B \wedge A = 1$, so $B \wedge A^n = 1$, therefore $B \wedge 1$, so $B$ is a constant, and $\beta = A/B$ is a polynomial.
...
If $A = \lambda t,\  \lambda \in k^*$, then $\lambda^n t^n + (1- \lambda) t = 0, \ n>1$ shows that $t$ is algebraic over $k$, in contradiction with the definition of $t$ as transcendental variable over $k$.
...
If $h = x^m-1$, then $h' = m x^{m-1}$, where $m$ is invertible in $F$, so the BŽzout's relation
$m^{-1} h' + hq^{-1}= x(m x^{m-1}) -(x^m-1) =1$ proves that $h\wedge h'=1$, so $h$ is separable. Let $K$ a splitting field of $h$ over $L$. Let 
