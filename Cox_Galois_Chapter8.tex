...
\begin{enumerate}
\item[(a)]
Let $f=x^m-1,\ m\in \N^*$. Then $f'=m x^{m-1}$ is relatively prime with  $f$. Indeed $m\neq 0$ in $L$ since the characteristic of $L$ is not $0$, and $-f +m^{-1} x f'= -x^m+1 + x^m = 1$ is a BŽzout's relation between $f$ et $f'$. Therefore (Prop. 5.3.2), $f$ is a separable polynomial, so $x^m-1$ has $m$ distinct roots in $M$,  the splitting field of $f$ over $L$.
