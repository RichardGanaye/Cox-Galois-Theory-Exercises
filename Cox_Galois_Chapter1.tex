...
Si $\sigma \in S_3$, we define $$\sigma \cdot \sqrt{\Delta}  = (x_{\sigma(1)}-x_{\sigma(2)})(x_{\sigma(1)}-x_{\sigma(3)})(x_{\sigma(2)}-x_{\sigma(3)}).$$
...
$f$ is strictly increasing on $]-\infty,\beta]$ and on $[\alpha, +\infty[$, strictly decreasing sur $[\beta,\alpha]$. $\beta$ is a local maximum, and $\alpha$ a local minimum. As $f$ is decreasing on  $[\beta,\alpha]$, $f(\beta) > f(\alpha)$.
...
{\it 
The goal of this exercice is to prove Theorem 1.3.3. Let $y^3+py+q=0$ be a cubic equation with positive discriminant. Consider the substitution $y = \lambda t$, which transforms the given equation into $\lambda^3 t^3 +\lambda pt+q = 0$.
\begin{enumerate}
...
\end{align*}
Therefore the polynomial $$h(t) = t^4 -\frac{3}{4} - \frac{1}{4} \cos(3\theta)$$ has three roots :  $\cos(\theta), \cos(\theta+ \frac{2\pi}{3}),\cos(\theta+ \frac{4\pi}{3})$. These roots are real, and distinct, since $\Delta>0$.
