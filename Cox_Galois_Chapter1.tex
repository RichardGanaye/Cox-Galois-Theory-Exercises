...
$p/3=-1$ and $q/2=0$, so the Cardan's formulas give, with the choice $\sqrt{-1} = i$, 
...
\begin{align*}
&y_1 = z_1 + z_2, &y_4 = \omega z_1+\omega z_2 = \omega y_1,\ \ &y_7 = \omega^2z_1+\omega^2 z_2 = \omega^2 y_1\\
&y_2=\omega z_1+\omega^2 z_2, &y_5 = \omega^2z_1+z_2=\omega y_2, \ \ &y_8 = z_1+\omega z_2=\omega^2 y_2\\
&y_3= \omega^2z_1+\omega z_2, &y_6 = z_1+\omega^2z_2 = \omega y_3 \ \ &y_9 = \omega z_1+z_2 = \omega^2 y^3
\end{align*}
...
\begin{proof}
\begin{enumerate}
\item[(a)] If $p=0$,  by (1.7) and Ex. 1.2.1, $\{z_1,z_2\} = \{0,\sqrt[3]{-q}\}$ :  $z_1,z_2$ are the solutions of  the equation $ 0 = z^6+qz^3-\frac{p^3}{27} = z^6+qz^3$.
...
Si $\sigma \in S_3$, we define $$\sigma. \sqrt{\Delta}  = (x_{\sigma(1)}-x_{\sigma(2)})(x_{\sigma(1)}-x_{\sigma(3)})(x_{\sigma(2)}-x_{\sigma(3)}).$$

If $\tau = (1\ 2)$, 
 $$\tau. \sqrt{\Delta}  = (x_2-x_1)(x_2-x_3)(x_1-x_3) = -\sqrt{\Delta}.$$
 
Same result if $\tau = (1\ 3)$, or $\tau = (2\ 3)$.

If $\sigma = (1\  2\  3)$,
$$\sigma. \sqrt{\Delta}  = (x_2-x_3)(x_2-x_1)(x_3-x_1) = \sqrt{\Delta},$$
with the  same result if $\sigma = (1 3 2)$ (and also if $\sigma$ is identity).

In conclusion, 

$\sigma.\sqrt{\Delta}  = \sqrt{\Delta} $ if $\sigma$ is even,

$\sigma.\sqrt{\Delta}  =- \sqrt{\Delta} $ if $\sigma$ is odd.
\end{proof}
...
The goal of this exercise is to give a differnet proof of (1.22).
\begin{enumerate}
...
\item[(e)] By combinig parts (c) and (d), conclude that $\Delta = -4p^3-27q^2$.
\end{enumerate}
}
...
{\it Let $f(y) = y^3+py+q$. The purpose of Exercises 2 to 5 is to prove Theorem 1.3.1 geometrically using graphing techniques. The proof breaks up into three cases corresponding to $p>0,p=0$, and $p<0$. This exercice will consider the case $p>0$.
\begin{enumerate}
...
Moreover $\lim\limits_{y\to +\infty} f(y) = +\infty$, and $\lim\limits_{y\to -\infty} f(y) = -\infty$, so there exists $x_1$ such that $f(x_1)<0$ and there exists $x_2$ such that $f(x_2)>0$. As $f$ is a continuous function, the intermediate values theorem gives the existence of a real root of $f$. As $f$ is injective, this is the only real root.
...
\begin{enumerate}
 ...
 \item[(b)]
For all $y \in \mathbb{R}$, $f'(y) = 3y^2 \geq 0$ et $f'(y) = 0$ only if $x=0$, so $f$ is strictly increasing. With the same argument already given in Ex. 1.3.2, $f$ has a unique real root, written $\sqrt[3]{-q}$.
\end{enumerate}
\end{proof}
...
\item[(a)]
$f'(y) = 3y^2+p < 0 \iff y^2<-p/3 \iff \alpha < f'(y) < \beta\  (\alpha = \sqrt{-p/3} = -\beta)$.

$f$ is stricty increasing on $]-\infty,\beta]$ and on $[\alpha, +\infty[$, strictly decreasing sur $[\beta,\alpha]$. $\beta$ is a local maximum, and $\alpha$ a local minimum. As $f$ is decreasing on  $[\beta,\alpha]$, $f(\beta) > f(\alpha)$.
...
\item[(c)] As these cases are mutually exclusive, (b) prove that $f$ has 3 real roots iff $f(\alpha)f(\beta)>0$. 
...
Reciprocally, if $\Delta>0$, then $p<0$ and $(\alpha)f(\beta)<0$. By (c), we know that $f$ has 3 real roots.
...
Then $z_1^3 = \overline{z_2}^3$, so $z_1 = \omega^k \, \overline{z_2},\ k=0,1,2$.
...
$$-\frac{q}{2}+ \sqrt{\left (\frac{q}{2}\right)^2+\left (\frac{p}{3}\right)^3} = 2 + \sqrt{4 - 125} = 2 + i \sqrt{11}.$$
...
\begin{proof}
Let $g(t) = 4 t^3-3t -\cos(3\theta) = \frac{1}{4} f(t)$, where 
...
{\it 
The goal of this exercice is to prove Theorem 1.3.3. Let $y^3+py+q=0$ be a cubic equation with poistive discriminant. Consider the substitution $y = \lambda t$, which transforms the given equation into $\lambda^3 t^3 +\lambda pt+q = 0$.
\begin{enumerate}
...
Then $h(t)$ has the same roots that $g(t) = f(\lambda t)$.

$-\frac{4p}{\lambda^2} = 3 \iff \lambda^2 = -\frac{4}{3} p$

If we take $\lambda = 2\sqrt{\frac{-p}{3}}$, then $-\frac{4q}{\lambda^3} = \frac{-4q}{-\frac{8p}{3}\sqrt{-\frac{p}{3}}} = \frac{3\sqrt{3}q}{2p\sqrt{-p}}$
...
Let $t$ a root of $h(t)$, then $u(t) = 4t^3 - 3 t = \cos(3\theta) \in [-1,1]$. By the properties of the function $u$ on $[-1,1]$, $t \in [-1,1]$, so there exists $\alpha \in \mathbb{R}$ tel que $\cos(\alpha) = t$.
\begin{align*}
...
\end{align*}
The polynomial $$h(t) = t^4 -\frac{3}{4} - \frac{1}{4} \cos(3\theta)$$ has so three roots :  $\cos(\theta), \cos(\theta+ \frac{2\pi}{3}),\cos(\theta+ \frac{4\pi}{3})$. These roots are real, and distinct, since $\Delta>0$.
...
We have proves Theorem  1.3.3 :

{\it Let $y^3+py+q$ a polynomial with real coefficients and positive discriminant. Then $p<0$, and the root of the equation are
...
\item[(b,c)] 
We choose then the cube root $z_1 = \frac{1}{2} e^{i \theta}$. 
...
{\it Example 1.3.2 discusses Bombeli's discovery that $\sqrt[3]{2+11i} = 2+i$. But not all cube roots can be expressed so simply. This exercise will show that $\sqrt[3]{4+\sqrt{11}i}$ is not of the form $a+b\sqrt{11}i$ for $a,b \in \Z$.
\begin{enumerate}
\item[(a)] Suppose that $4 + \sqrt{11}i = (a+ b \sqrt{11}i)^3$ for some $a,b\in \Z$. Show that this implies that $4 = a^4 - 33 ab^2$ and $1 = 3a^2b - 11 b^3$.
...
&=-4b^2(x_1-x_2)^2\vert x_1-a-bi\vert^2 \vert x_2 -a -bi \vert^2 <0
\end{align*}
...
&=(2ib)^2(2id)^2 \vert x_1-x_3 \vert^2\vert x_1-x_4 \vert^2\\
&=16b^2d^2 \vert x_1-x_3 \vert^2\vert x_1-x_4 \vert^2 >0\\
\end{align*}
...
With a calculator, an approximation at $10^{-10}$ of the real root $x_1$ est $x_1 \simeq 1.3688081078$.
...
To verify the Fibonacci' result  in 1225, we compute a decomposition of $x_1$ with basis 60 with the following Maple program (or Sage program) :
