...
\begin{proof}
Let $K = \Q[\omega]$, $z_1 = \frac{1}{3}(x_1+\omega^2 x_2+\omega x_3) \in \Q[\omega][x_1,x_2,x_3]$,  and $\theta(z) \in \Q[\omega][z]$ be the resolvent polynomial of $z_1$.
...
and these six elements are distinct in $K[x_1,x_2,x_3]$.
...
Since $|H(y_1)| = |\mathrm{Stab}_{S_n}(y_1)| = 8$ (see Part (d)), $|{\cal O}_{y_1} |= 3$, so $y_1,y_2,y_3$ are all the elements of ${\cal O}_{y_1}$.
...
We obtain this by hand, or with the Dimino algorithm, or with the Sage instructions:
\begin{verbatim}
...
\item[(d)] If we use the number the 4 consecutive summits of the square $(1,3,2,4)$, then $H(y_1)$ is isomorphic to the group generated by the rotation of angle $\pi/2$ corresponding to  $(1\,3\,2\,4)$ and the reflection relative to the diagonal $(3,4)$ corresponding to $(1\,2)$, and this the dihedral group $D_8$. 
$$H(y_1) \simeq D_8.$$
\ee
\end{proof}
...
\item[(b)] Now let $y = x_1x_2 +x_3x_4$, and define the square root in (12.11) using (12.12). Show that the roots of (12.11) are $x_1,x_2$ for the plus sign and $x_3x_4$ for the minus sign.
\ee
}
...
&=\frac{1}{4}\left[(x_1+x_2)^2+(x_3+x_4)^2 - 2(x_1+x_2)(x_3+x_4)^2 - 2(x_1+x_2)(x_3+x_4)\right]\\
...
Let $u,v$ be the roots of this equation, and $S = u+v, P= uv$ the sum and product of these roots. Then
\begin{align*}
...
We show that $H = H(\varphi)$:
 ...
 \item[$\bullet$] If $\tau \in H(\varphi)$,  $\tau \cdot \varphi = \varphi$, so
 ...
 Since the exponents $a_1,\ldots,a_n$ are distinct, the $k$ terms of $\sum_{\chi \in H} \chi \cdot m$, where $k = |H|$,  are distinct, so there exist exactly one term in the right member which is the same as the term $ x_1 ^{a_{\tau^{-1}(1)}} \cdots x_n ^{a_{\tau^{-1}(n)}}$ of the left member corresponding to $\sigma = e$, so there exists $\chi \in H$ such that
...
\item[(b)] Since $L \subset K(\sqrt{\Delta}) \subset M$, the Galois correspondence being order reversing,
...
Write $\rho =  (1\, 2 \ldots n)  \in S_n$ and $H = \langle \rho \rangle$. Let $\tau H$ any coset relative to $H$, with $\tau \in S_n$. We must prove that there exists a unique $\sigma \in \tau H$ such that $\sigma(1) = 1$
	...
	Since $\sigma'(1) = 1$, we have $\sigma(\rho^l(1)) = 1 = \sigma(1)$. $\sigma$ is one-to-one, so $\rho^l(1) = 1$, therefore $l\equiv 0 \pmod n$, so $\rho^l =  e$ and $\sigma = \sigma'$.
	\ee
...
Let $\varphi = 1$ ( or any element in $K$). Then $\varphi$ is invariant under $S_n$, so $\psi$ is fixed by every permutation fixing $\varphi$. By Theorem 12.1.6. $\psi$ is a rational function of $\varphi$ with coefficients in $K = F(\sigma_1,\ldots,\sigma_n)$, i.e., $\psi \in K(\varphi) = K(1) = K$. So $\psi \in F(\sigma_1,\ldots,\sigma_n)$.
...
\end{array}
$$
Le formulas are NOT surprisingly different.
\ee
\end{proof}
...
\right.
$$
Let $g, g' \in G$, $\sigma = \psi(g), \sigma' = \psi(g')$. For all $i, 1\leq i \leq n$,
...
Since $H = eHe^{-1} \subset \bigcap_{h\in G} hHh^{-1}$, $H\subset N$.
