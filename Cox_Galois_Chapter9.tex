...
\begin{proof}
If $[i]$ has a multiplicative inverse in the ring $\Z/n\Z$, then there exists $[j] \in \Z/n\Z$ such  that $[i][j] = [ij] = 1$, so $ij \equiv 1\ [n]$. Thus there exists $k\in \Z$ such that $ij - k n =1$. This Bézout's relation between $i$ and $n$ shows that $i\wedge n=1$.

Conversely, if $i\wedge n=1$, by  Bézout's Theorem, there exist integers $j,k$ such that $ij-kn=1$, so $[i] [j] = [1]$, and $[i]$ has a multiplicative inverse $\Z/n\Z$.
...
\begin{proof}
Let $\U_n$ be the group of $n$throots of unity in $\C$. Then $\U_n = \langle \zeta_n \rangle$, where $\zeta_n = e^{2i\pi/n}$. Write $o(x)$ the order of an element $x\in G$. Then $o(x)= \vert \langle x \rangle \vert$.
...
{\it The Möbius function is defined for integers $n\geq 1$ by
$$
\mu(n) = 
\left\{
\begin{array}{ll}
...
{\it Let $\mu$ be the Möbius function defined in Exercise 14. Prove that
...
&=-4.
\end{align*}
The minimal polynomial over $\Q$ of the 8-périods $(8,1),(8,3)$ is so
$$(x-(8,1))(x-(8,3)) = x^2 +x-4.$$
...
\item[(b)] Use the numerical computations of part (a) and the quadratic polyomials of Exercice 8 to show that
\begin{align*}
...
Les $f$-périodes sont linéairement indépendantes sur $\Q(\zeta_m)$.
\end{enumerate}
\end{proof}
...
\begin{proof}
$H_1 = \{[1]\}$, and the coset of $[a] \in (\Z/p\Z)^*$ is $[a]H_{1} = \{[a]\}$, so  the 1-périodes $(1,a)$ are the powers of $\zeta_p$: $$(1,a) = \zeta_p^a.$$
\end{proof}
