...
$\bullet$ Reciprocally, if $F[x] = I = \langle g \rangle$, then $ 1 \in \langle g \rangle$, so $1 = g u, u \in F[x]$, hence $0 = \deg(g)+\deg(u)$, therefore $\deg(g) = 0$, so $g \in F$ is a nonzero constant.
...
$\bullet$ Reciprocally, if $\langle f \rangle = \langle g \rangle,$  then $g \in \langle f \rangle$ , $g = v f, v \in k[x]$, thus $g = v gh$. As $f = gh\neq 0, g \neq 0$, thus $1 = vh$, therefore $h \in F^*$ is a constant.
...
\begin{proof}
Let $x \in F$. If $x\ne 0$, then $x.x^{-1} = 1$, thus $\varphi(x) \varphi(x^{-1}) = 1$, so $\varphi(x) \ne 0$.
For all $x\in F$, $x \ne 0 \Rightarrow \varphi(x) \ne 0$, therefore $\varphi(x) = 0 \Rightarrow x =0$, so $\ker(\varphi) = \{0\}$ and $\varphi$ is injective.

Consequently, the corestriction $F \to \varphi(F), x \mapsto \varphi(x)$ is a bijection, so it is an ring isomorphism : $\varphi : F \simeq \varphi(F)$.
\end{proof}
...
\begin{proof}
Let $a,b \in A = F[x]$. Suppose that $a+I = a'+I$ et $b+I = b'+I$.
...
Moreover $\varphi(1) = 1+I$ is the neutral element of $A/I$.
...
{\it Let $f \in F[x]$ be irrducible, and let $g+\langle f \rangle$ be a nonzero coset in the quotient ring $L = F[x]/\langle f \rangle$.
\begin{enumerate}
...
\begin{proof}
Supposons $f \in F[x]$ irrŽductible, et soit $L =F[x]/\langle f \rangle$ l'anneau quotient.
\begin{enumerate}
...
Let $h$ a common divisor of $f$ et $g$. Since $f$ is irreducible, either $u$ is a nonzero constant, or $u = k f, k \in F^*$ is associate to $f$. But in this last case, $f = k^{-1} u$ divides u, which divides $g$, so $f \mid g$, in contradiction with the hypothesis.

So the only common divisors of $f,g$ are the $nonzero constants$, so $f \wedge g = 1$.

By  BŽzout theorem, there exists polynomials $A,B \in k[x]$ such that
$$1 = A f + B g.$$
...
\begin{proof}
$-x(x+1) + (x^2+x+1) = 1$ is a BŽzout's relation between $x+1$ est $x^2+x+1$.

$f = x^2+x+1$has no root in $\mathbb{Q}$ is has degrre 2 : it is so irreducible on $\mathbb{Q}$, and 
...
$-x + \langle f \rangle$ is the iinverse of $x+1+\langle f \rangle$ in $F[x]/\langle f \rangle$.
\end{proof}
...
&=\sum\limits_{i=0}^n{\overline{a_i}x^i}  \sum\limits_{j=0}^n\overline{b_j}x^i\\
...
\item[(d)] Conlude that $a+bi$ has a square root in $\C$.
\end{enumerate}
}
...
&\iff (a+ ib)^2 = x+iy \ \mathrm{et} \ \vert a+ ib\vert ^2 = \vert x+iy\vert \\
&\iff a^2 - b^2 +2abi = x+iy\ \mathrm{et} \ a^2+b^2 = \sqrt{x^2+y^2}\\
...
The reciprocal is true, since these last equations imply
$$4a^2 b^2 = \left(  \sqrt{x^2+y^2}  + x\right) \left( \sqrt{x^2+y^2} -x\right) = x^2+y^2-x^2 = y^2,$$
and as  $\mathrm{sgn}(ab) =  \mathrm{sgn}(y)$, we conclude $2ab=y$. So we have proved the equivalence
...
Let $u:\mathbb{R} \to \mathbb{R}, x\mapsto u(x) = x^2 -a$.
...
By the Intermediate Value Theorem, there exists a unique $b\in \mathbb{R}^+$ such that $b^2 = a$ : $a$ has a real square root.
\end{proof}
...
If $-a \in P$, $a^2 = (-a)(-a) \in P$. By (b), every $a\in F$ verifies $a\in P$, or $a=0$, or $-a\in P$, we can conclude that 
\begin{align}
...
By (b) , les trois cas $a\in P$,  $a=0$, $-a\in P$ are mutually exclusive, thus $-1 \not \in P$.
...
Therefore $F(i) = F[x]/\langle x^2+1\rangle$ is a field, where $i = \dot{x}$, by Proposition 3.1.1.
...
If $\Delta >0$,
 $\Delta$ has a square root in $F$, by the definition of a real closed field (and the root 0 if $\Delta =0$).
If $\Delta <0$, $\Delta = (i\sqrt{-\Delta})^2$ has a square root in $F(i)$.
We will write $\sqrt{\Delta}$ one of these square roots. Then
 \begin{align*}
...
By definition of a real closed field, and by (b),
 ...
 $\bullet$ every elemen $a \in F(i)$ has a square root in $F(i)$,
...
\item[(b)]
$\bullet$ Suppose that every polynomial in $\mathbb{C}[x]$ has a root in $\mathbb{C}$.
...
By (a), $\overline{\alpha}$ is a root of $g$. $0 = g(\overline{\alpha}) = (\overline{\alpha} - \alpha) g_1(\alpha)$, and $\overline{\alpha} \neq  \alpha$, thus $g_1(\overline{\alpha})=0$, $g_1 = (x- \overline{\alpha}) h, h \in \mathbb{C}[x]$, therefore
...
$\bullet$ Reciprocally, suppose that every polynomial in $\R[x]$ is product of linear or quadratic factors with real coefficients.
...
Conclusion :if $F$ is a field, the two following propositions are equivalent :
\begin{enumerate}
