...
$\bullet$ Conversely, if $\langle f \rangle = \langle g \rangle,$  then $g \in \langle f \rangle$ , $g = v f, v \in k[x]$, thus $g = v gh$. As $f = gh\neq 0, g \neq 0$, thus $1 = vh$, therefore $h \in F^*$ is a constant.
...
By  BŽzout theorem, there exist polynomials $A,B \in k[x]$ such that
$$1 = A f + B g.$$
...
$f = x^2+x+1$ has no root in $\mathbb{Q}$ is has degree 2, therefore $f$ is irreducible on $\mathbb{Q}$, and consequently $\Q[x]/\langle f \rangle$ is a field.
 ...
 Moreover $-x(x+1) + (x^2+x+1) = 1$ is a BŽzout's relation between $x+1$ and $x^2+x+1$. This gives the following equality in $\Q[x]/\langle f \rangle$:
