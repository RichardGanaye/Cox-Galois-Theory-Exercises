...
\item[(c)]
 ...
 Let $u,v \in F^n \setminus \{0\}$. Since $u \ne 0$, we can compete $u$ in a base ${\cal B} = (u_1,\ldots,u_n)$ of $F_n$, where $u_1 = u$. Similarly, we can complete $v\ne 0$ in a base ${\cal B}' = (v_1,\ldots,v_n)$, where $v1 = v$. 

Since ${\cal B},{\cal B}' $ are two bases, there exists some bijective linear map $f : F^n \to F^n$ such that $f(u_i) = v_i,\ i=1,\ldots,n$.

Then the matrix $A = {\cal M}_{{\cal B},{\cal B}'}(f)$ of $f$ in the bases ${\cal B},{\cal B}' $  satisfies $A \in \mathrm{GL}(n,F)$, and $A \cdot u = v$. This proves that $\mathrm{GL}(n,F)$ acts transitively on $F^n \setminus\{0\}$.
