...
\item[(b)] The same first argument as in part (a) shows that the irreducible polynomial $f = x^6 + bx^3 + c \in F[x]$ is separable.
 If $\alpha$ is a root of $f$ in $L$, then $ \lambda = \alpha^3 \in L $ is a root of $g = x^2 +bx+c$. Let $\mu = -b-\lambda \in L$. Then $\mu$ is a root of $g$: 
