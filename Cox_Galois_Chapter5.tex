...
Moreover  $ \omega = \omega\sqrt[3]{2} / \sqrt[3]{2} \in \Q(\sqrt[3]{2},\omega \sqrt[3]{2},\omega^2 \sqrt[3]{2})$ et $\sqrt[3]{2} \in \Q(\sqrt[3]{2},\omega \sqrt[3]{2},\omega^2 \sqrt[3]{2})$. As $\Q(\omega,\sqrt[3]{2})$ is the smallest subfield of $\C$ containing thes two elements, 
...
The roots of $f$ are so $x_1,\ldots, x_n$, with possibly some repetition.
The splitting field of $f$ over $F$ is so $F(x_1,\ldots,x_n)$. As $x_i \in F,\ i=1,\ldots,n$, $F(x_1,\ldots,x_n) = F$.

The splitting field of $f$, which splits completely over $F$, is $F$.

Reciprocally, suppose that the splitting field of $f$ over $F$ is $F$. Let $x_1,\ldots,x_n$ the roots of $f$ in the splitting field of $f$. As this field is $F$, $x_1,\ldots ,x_n \in F$, thus
$$f = a(x-x_1)\cdots(x-x_n), \ x_i \in F,\ i=1,\ldots,n.$$
...
Let $f$ the minimal polynomial of $\alpha$ over $F$. Then $\deg(f) =[F(\alpha):F] =  2$, so $f = x^2+ax+b,\ a,b\in F$.

In an extension $K$ of $L$, $f$ splits completely, and $\alpha \in L \subset K$ is a root of $f$, so there exists $\beta \in K$ such that
$$f = x^2+ax+b = (x-\alpha)(x-\beta) = x^2 - (\alpha+ \beta)x + \alpha \beta.$$

Thus $\alpha + \beta = a \in L$, with $a\in F \subset L$, so $\beta = a -\alpha \in L$. Hence the splitting field of $f$ over $F$ is $F(\alpha,\beta) = L(\beta) = L$.
...
Reciprocally, $\omega \in S$, thus $\Q(\omega) \subset \Q(S)$.
...
The Schönemann-Eisenstein Criterion,  with $p=2$ applies to the polynomial $f= x^4-4x^2+2 = a_4x^4+a_3x^3+a_2x^2+a_1x+a_0$ 
...
As $\deg(f) = 3$, it is suffisant to prove the irreducibility of$f$ to show that $f$ has no root in $\F_3$.
...
In conclusion, $L = \F_3(\alpha)$ is the splitting field of $x^3-x+1$ over $\F_3$. Its degree is 3 over $\F_3$. As a vector space over $\F_3$, its dimension is 3, so  $L \simeq \F_3^3$ : its cardinality is so $3^3=27$.
\end{enumerate}
\end{proof}

\paragraph{Ex. 5.1.8}

{\it Let n be a positive integer. Then the polynomial $f = x^n-2$ is irreducible over $\Q$ by the Schönemann-Eisenstein Criterion for the prime 2.
\begin{enumerate}
...
\begin{enumerate}
\item[(a)]
Let $f \in F[x], \deg(f)=n>0$, and $L$ the splitting field of $f$ over $F$.

Suppose that $f$ is reducible over $F$, and we show then that $[L:F]<n!$.

$f =gh$, where $1\leq k = \deg(g) \leq n-1$ (then $\deg(h)=n-k$).

The roots $\alpha_1,\cdots,\alpha_k$ of $g$, and the roots $\beta_1,\cdots,\beta_{n-k}$ of $h$, are the roots of $f$. They are thus in $L$, and
$$L = F(\alpha_1,\cdots,\alpha_k,\beta_1,\cdots,\beta_k).$$
...
$F \subset K \subset L$ and $\alpha_1,\ldots,\alpha_n \in L$, thus $K(\alpha_1,\ldots,\alpha_n) \subset L = F(\alpha_1,\ldots,\alpha_n) \subset K(\alpha_1,\ldots,\alpha_n)$. Consequently, $L = K(\alpha_1,\ldots,\alpha_n)$, and $f$ splits completely over $K$ since $c \in F \subset K$. $L$ is the splitting field of $f$ over $K$.

Note : so, if $F\subset K \subset L$, and if $F \subset L$ is a normal extension, so is $K \subset L$.
\end{proof}
...
$[L_1:F_1] < \infty$. Let $(f_1,\ldots,f_d)$ a basis of $L_1/F_1$. We show that $(\overline{\varphi}(f_1), \ldots,\overline{\varphi}(f_d))$ is a basis of $L_2/F_2$.
...
As the restriction of $\overline{\varphi}$ to $F_1$ is $\varphi$, $b_i = \overline{\varphi}(a_i)$. $\overline{\varphi}$ being a ring homomorphism,
$$0= \sum_{i=1}^d b_i \overline{\varphi}(f_i) = \sum_{i=1}^d \overline{\varphi}(a_i) \overline{\varphi}(f_i) = \overline{\varphi}\left (\sum_{i=0}^d a_i f_i\right).$$
As the kernel of $\overline{\varphi}$ is $0$, $\sum_{i=0}^d a_i f_i =0$, where the family $(f_i)_{0\leq i \leq d}$ is free, thus $a_1=\cdots=a_d=0$, and since $b_i = \varphi(a_i)$, $b_1=\cdots=b_d=0$. So the family $(\overline{\varphi}(f_i))_{1\leq i \leq d}$ is free.

{\medskip}

 $\bullet$ Let $y $ any element in $L_2$. As $\overline{\varphi}$ is surjective, there exists $x\in L_1$such that $y =\overline{\varphi}(x)$.
 $(f_1,\cdots,f_d)$ being a basis, there exists $(a_0,\cdots,a_d) \in F_1^d$ such that  $x = \sum_{i=0}^d a_i f_i$.
 
 Then $$y = \overline{\varphi}(x) = \sum_{i=0}^d \overline{\varphi}(a_i) \overline{\varphi}(f_i) =  \sum_{i=0}^d \varphi(a_i) \overline{\varphi}(f_i) =  \sum_{i=0}^d b_i\overline{\varphi}(f_i),$$
...
$x^4 - 2$ is an irreducible polynomial over $\Q$ $\Q$ by Schönemann-Eisenstein Criterion with $p=2$.
...
Reciprocally, suppose that every $\alpha \in L$ is such that its minimal polynomial splits completely over $F$.

Let $g \in F[x]$ any irreducible polynomial, and $\alpha$ a root of $g$ in $L$. Then $g$ is the minimal polynomial of $\alpha$ over $L$. So $g$ splits completely over $L$ by hypothesis. Hence every irreducible polynomial  $g$ which has a root in $L$ splits completely over $F$. So the extension  $F\subset L$ is normal.
\end{proof}
...
\begin{enumerate}
\item[(a)]
As $\Q(\zeta_n)$ contient $\zeta_n^k$ pour tout $k \in \Z$, 
...
\item[(b)]
By \eqref{eq5.3.1:1}, lthe definition of the product of two polynomials gives
...
\item[(a)] Here $n \geq 1$.
 ...
 $x(nx^{n-1}) - n(x^n-1) = n = xf'-nf$ is a Bézout's relation  between $f$ and $f'$, which proves that $f\wedge f'=1$. So $f$ is a separable polynomial, so the $n$ roots of $f$ in its splitting field, which are the $n$th roots of unity, are distinct.
...
\begin{proof}
Let $\Delta = \Delta(\sigma_1,\cdots,\sigma_n) \in F(\sigma_1,\cdots,\sigma_n) \subset F(x_1,\cdots,x_n)$ the discriminant.

Let $f =x^n + a_1 x^{n-1}+ \cdots+a_0 \in \Z[x]$a monic nonconstant polynomial.

In the section 2.4, $\Delta(f)$ is defined by 
$$\Delta(f) = \Delta(-a_1,\cdots,(-1)^i a_i, \cdots,(-1)^n a_n).$$
obtained with the evaluation homomorphism defined by $\sigma_i \mapsto (-1)^ia_i$, which sends $\tilde{f} = x^n-\sigma_1 x^{n-1}+\cdots+(-1)^n \sigma_n$ on $f$, and $\Delta$ sur $\Delta(f)$.
...
$\Delta(-\overline{a}_1,\cdots,(-1)^i \overline{a}_i, \cdots,(-1)^n \overline {a}_n)$ is the reduction modulo $p$ of $\Delta(-a_1,\cdots,(-1)^i a_i, \cdots,(-1)^n a_n)$, that is $\Delta(f_p)$ is the reduction modulo $p$ of $\Delta(f)$ : $$\Delta(f_p) = [\Delta(f)]_p.$$
\end{proof}
...
\begin{proof}
Soit $F$ un corps de caractéristique $p$, et $f \in F[x]$ irréductible sur $F$, de degré $n\geq 1$.
\begin{enumerate}
\item[(a)]
...
\item[(c)]
By reductio ad absurdum, if $g_1$ was reducible, $$g_1 = u v,\ g_1,g_2 \in F[x], 1\leq \deg(u), 1\leq deg(v) .$$
...
\begin{enumerate}
\item[(a)]
The Exercice 4.2.9, applied to the field $k(u)$,  shows that $x^2-t$  has no root in $F  = k(t,u) = k(u) (t)$, so it is irreducible over $F$. So $x^2 -t$ is the minimal polynomial of $\alpha$ over $F$. 

In $L$, $x^2 - u = (x-\alpha)(x+\alpha)$, and $\alpha \neq -\alpha$, otherwise $2 \alpha = 0$, with $2=-1\neq 0$ in $k$, and $\alpha \neq 0$ since $\alpha^2 = t \ne 0$.
...
\begin{enumerate}
\item[(a)]
As the characteristic is $p$, $f =x^p -a = (x -\alpha)^p$ has only one root $\alpha$. lThe splitting field of $f$ over $F$ is so $F(\alpha)$, and $f$ being the minimal polynomial of  $\alpha$ over $F$, $[F(\alpha):F] = \deg(f) = p$.
...
\item[(c)]
Write $b = \beta^p \in F$. $\beta$ is a root of $x^p -b \in F[x]$.
...
$\bullet$ Reciprocally, suppose that the minimal polynomial over $F$ of every $\alpha \in L$ is of the form $f=x^{p^e} -a, a\in F, e\geq 0$.
...
\item[(b)]
{\bf Lemma : } {\it If $F \subset L$ is a finite purely inseparable extension,  and if $L \subset K \subset L$, then $K \subset L$ is purely inseparable.}
...
If $f, f'$ have a common root $\alpha$ in an extension  $L$ of $F$, then $x-\alpha$ divides $f$ dans $L[x]$, and also $f'$, so divides their gcd in  $L[x]$, and so $\mathrm{gcd}(f,f') \ne 1$ (we know that the gcd is the same in $·[x]$ and in $L[x]$).

We have proved that if $f\wedge f' = 1$, then $f,f'$ have no common root in any extension of $F$.

Reciprocally, if $f \wedge f' \neq 1$, then $f,f'$ have a common nonconstant factor  $g \in F[x]$. 
...
$\bullet$ Let $\alpha$  any element of $L$. As $\alpha$ is separable over $F$, the minimal polynomial $f \in F[x]$ of  $\alpha$ over $F$ is separable, thus $f$ has only simple roots in a splitting field $R$ of $f$ over $L$ (which is also a splitting field of $f$ over $F$ or over  $K$). The minimal polynomial $f_K$ of $\alpha$ over $K$ divides $f$ (since $f(\alpha) = 0$ and $f\in F[x] \subset K[x]$). As $f_K \mid f$, the order of multiplicity of a root of $f_K$ is at most the order of multiplicity of this root in $f$, thus all the roots of $f_K$ in the splitting field $R$ are simple, thus $\alpha$ is separable over $K$. Therefore the extension $K \subset L$ is separable.
\end{proof}
...
$\bullet$ Reciprocally, suppose that $$f(\beta) = f'(\beta) = \cdots = f^{(m-1)}(\beta) = 0, f^{(m)}(\beta)\neq 0.$$
...
\item[(c)]
As $(L^*,\times)$ is a group of cardinality $m = \vert L \vert - 1$, the Lagrange's Theorem show that the order of every $\gamma  \in L^* = \{1,\alpha,\alpha^2,\cdots,\alpha^{m-1}\}$ satisfies $\gamma^m =1$, and so is a root of $x^m-1$.
...
{\it In the extension $F \subset L$ of example 5.4.4, we have $F=k(t,u)$, where $k$ has characteristic $p$ and $L$ is the splitting field of $(x^p-t)(x^p-u) \in F[x]$. We also have $\alpha,\beta \in L$ satisfying $\alpha^p=y, \beta^p=u$. Prove the following properties of $F \subset L$:
\begin{enumerate}
...
The polynomial $x^p -u$ has no root in  $k(t,u,\alpha) = F(\alpha)$ by Exercise 4.2.9. applied to the field $k(t,\alpha)$. Moreover, $p$ is prime, so Proposition 4.2.6 shows that $h=x^p-u$ is irreducible over $F(\alpha)$. $h$ is so the minimal polynomial of $\beta$ sur $F(\alpha)$. Consequently, 
...
\begin{proof}
The proof of $F(\alpha,\beta) = F(\alpha+\lambda \beta)$ ises only 5.17 (5.16 is used only to prove the separability of $\alpha+\lambda \beta$).
...
\begin{proof}
Let a finite extension $F\subset L = F(\alpha_1,\cdots,\alpha_n)$, where $\alpha_1,\cdots,\alpha_{n-1}$ are separable over $F$ (but not $\alpha_n$). The Primitive Element Theorem 5.4.1shows that $F(\alpha_1,\cdots,\alpha_{n-1})$ has a primitive element $\beta$ separable over $F$.
...
\begin{proof}
Here $F=k(t,u)\subset L$, where the characteristic of $k$ is 3, and $L$ lis the splitting field of $(x^2-t)(x^3-u)$, and $\alpha,\beta \in L$ such that $\alpha^2 = t, \beta^3 = u$.
...
\end{align*}
have the root $\alpha$, since $g(\alpha) = 0, f(\beta) = 0$, and are both in ${F(\alpha+\beta)[x]}$.
...
Therefore $\deg(h) = 1$, and $h(x) =\mathrm{pgcd}(g(x),f(\alpha+\beta - x)) = x-\alpha$.
