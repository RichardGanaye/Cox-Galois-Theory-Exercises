...
Moreover  $ \omega = \omega\sqrt[3]{2} / \sqrt[3]{2} \in \Q(\sqrt[3]{2},\omega \sqrt[3]{2},\omega^2 \sqrt[3]{2})$ et $\sqrt[3]{2} \in \Q(\sqrt[3]{2},\omega \sqrt[3]{2},\omega^2 \sqrt[3]{2})$. As $\Q(\omega,\sqrt[3]{2})$ is the smallest subfield of $\C$ containing these two elements, 
...
$x^4 - 2$ is an irreducible polynomial over $\Q$ by Schönemann-Eisenstein Criterion with $p=2$.
...
obtained by applying to $\Delta(\sigma_1,\cdots,\sigma_n)$ the evaluation homomorphism defined by $\sigma_i \mapsto (-1)^ia_i$, which sends $\tilde{f} = x^n-\sigma_1 x^{n-1}+\cdots+(-1)^n \sigma_n$ on $f$, and $\Delta$ sur $\Delta(f)$.
...
The polynomial $x^p -u$ has no root in  $k(t,u,\alpha) = F(\alpha)$ by Exercise 4.2.9. applied to the field $k(t,\alpha)$. Moreover, $p$ is prime, so Proposition 4.2.6 shows that $h=x^p-u$ is irreducible over $F(\alpha)$. Therefore $h$ is the minimal polynomial of $\beta$ sur $F(\alpha)$. Consequently, 
